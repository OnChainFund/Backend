...
# The following code is adapted from the original paper:
# Title: Deep learning Statistic Arbitrage
# Author: Jorge Guijarro-Ordonez
# Availability: https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3862004
#               https://github.com/gregzanotti/dlsa-public
# """
...
class CNNTransformer(nn.Module):
    ...
    def forward(self, x):  # x has dimension (N,T)
        ...
        if self.use_transformer:
            ...
            x = self.linear(x[-1, :, :])  ##last time period
            S, _ = x.size()
            x = x.reshape((int(S / self.num), self.num))
            # x = self.softmax(x)
        if self.out == "tanh":
            x = self.tanh(x)
            x = x / (torch.sum(torch.abs(x), dim=1, keepdims=True) + 1e-5)
        elif self.out == "softmax":
            x = self.softmax(x)
        elif self.out == "log":
            x = torch.sign(x) * torch.log(1 + torch.abs(x))
            x = x / (torch.sum(torch.abs(x), dim=1, keepdims=True) + 1e-5)
        if self.out == "origin":
            x = x / (torch.sum(torch.abs(x), dim=1, keepdims=True) + 1e-16)
        return x
